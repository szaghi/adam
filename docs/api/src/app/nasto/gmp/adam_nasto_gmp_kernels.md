...
```mermaid
graph LR
  adam_nasto_gmp_kernels["adam_nasto_gmp_kernels"] --> adam_nasto_gmp_cns_kernels["adam_nasto_gmp_cns_kernels"]
  adam_nasto_gmp_kernels["adam_nasto_gmp_kernels"] --> adam_weno_object["adam_weno_object"]
  adam_nasto_gmp_kernels["adam_nasto_gmp_kernels"] --> penf["penf"]
```

## Contents

- [compute_fluxes_convective_gmp](#compute-fluxes-convective-gmp)
- [compute_fluxes_difference_gmp](#compute-fluxes-difference-gmp)
- [compute_fluxes_diffusive_gmp](#compute-fluxes-diffusive-gmp)
- [compute_umax_gmp](#compute-umax-gmp)
- [set_bc_q_gpu_gmp](#set-bc-q-gpu-gmp)
...
```fortran
subroutine compute_fluxes_convective_gmp(dir, blocks_number, ni, nj, nk, ngc, nv, weno_s, weno_a_gpu, weno_p_gpu, weno_d_gpu, weno_zeps, ror_number, ror_schemes_gpu, ror_threshold, ror_ivar_gpu, ror_stats_gpu, g, q_aux_gpu, fluxes_gpu)
```
...
flowchart TD
  ...
  style compute_fluxes_convective_gmp fill:#3e63dd,stroke:#99b,stroke-width:2px
```
...
```fortran
subroutine compute_fluxes_difference_gmp(blocks_number, ni, nj, nk, ngc, nv, ib_eps, dx_gpu, dy_gpu, dz_gpu, flx_gpu, fly_gpu, flz_gpu, phi_gpu, dq_gpu)
```
...
flowchart TD
  ...
  style compute_fluxes_difference_gmp fill:#3e63dd,stroke:#99b,stroke-width:2px
```
...
```fortran
subroutine compute_fluxes_diffusive_gmp(blocks_number, ni, nj, nk, ngc, nv, mu, kd, q_aux_gpu, dx_gpu, dy_gpu, dz_gpu, flx_gpu, fly_gpu, flz_gpu)
```
...
flowchart TD
  ...
  style compute_fluxes_diffusive_gmp fill:#3e63dd,stroke:#99b,stroke-width:2px
```
...
flowchart TD
  ...
  style compute_umax_gmp fill:#3e63dd,stroke:#99b,stroke-width:2px
```
...
```fortran
subroutine set_bc_q_gpu_gmp(BC_EXTRAPOLATION, BC_INFLOW, nv, ngc, cv, R, local_map_bc_gpu, fec_1_6_array_gpu, q_bc_vars_gpu, q_gpu)
```
...
| `local_map_bc_gpu` | integer(kind=[I8P](/api/src/third_party/PENF/src/lib/penf_global_parameters_variables)) | in |  | Local map for BC ghost cells. |
...
flowchart TD
  ...
  style set_bc_q_gpu_gmp fill:#3e63dd,stroke:#99b,stroke-width:2px
```
