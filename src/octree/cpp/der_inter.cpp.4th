...
void der1(float *** v_in, float *** v_out, int dir, float delta[3], int nt[3], int ng[3]) {
    ...
    if(dir == 1) {
        float invdelta = 1./(delta[0]);
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta * (1./12.*v_in[i-2][j][k] -  2./3.*v_in[i-1][j][k]+
                                                  2./3.*v_in[i+1][j][k] - 1./12.*v_in[i+2][j][k]);
    } else if(dir == 2) {
        float invdelta = 1./(delta[1]);
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta * (1./12.*v_in[i][j-2][k] -  2./3.*v_in[i][j-1][k]+
                                                  2./3.*v_in[i][j+1][k] - 1./12.*v_in[i][j+2][k]);
    } else if(dir == 3) {
        float invdelta = 1./(delta[2]);
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta * (1./12.*v_in[i][j][k-2] -  2./3.*v_in[i][j][k-1]+
                                                  2./3.*v_in[i][j][k+1] - 1./12.*v_in[i][j][k+2]);
    }
}
...
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta2 * (-1./12.*v_in[i-2][j][k] + 4./3.*v_in[i-1][j][k]+
                             -5./2.*v_in[i][j][k]+4./3.*v_in[i+1][j][k] - 1./12.*v_in[i+2][j][k]);
    } else if(dir == 2) {
...
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta2 * (-1./12.*v_in[i][j-2][k] + 4./3.*v_in[i][j-1][k]+
                             -5./2.*v_in[i][j][k]+4./3.*v_in[i][j+1][k] - 1./12.*v_in[i][j+2][k]);
    } else if(dir == 3) {
...
#pragma omp parallel for collapse(2)
        for(int i=ngx;i<ngx+nx;i++) 
            for(int j=ngy;j<ngy+ny;j++) 
                for(int k=ngz;k<ngz+nz;k++)
                    v_out[i][j][k] = invdelta2 * (-1./12.*v_in[i][j][k-2] + 4./3.*v_in[i][j][k-1]+
                             -5./2.*v_in[i][j][k]+4./3.*v_in[i][j][k+1] - 1./12.*v_in[i][j][k+2]);
    }
}
