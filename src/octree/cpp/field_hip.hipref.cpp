...
void allocate_dev(std::unordered_map<std::string, Field> & field_map, Fields_cuda & fields_cuda, 
    ...
    hipMalloc((void **)&(fields_cuda.dx_dev), max_blocks*sizeof(float));
    hipMalloc((void **)&(fields_cuda.dy_dev), max_blocks*sizeof(float));
    hipMalloc((void **)&(fields_cuda.dz_dev), max_blocks*sizeof(float));
    hipMalloc((void **)&(fields_cuda.level_dev), max_blocks*sizeof(int));

    size_t allblocks_size_f1 = block_size_f1*max_blocks;
    size_t allblocks_size_f1_ref = block_size_f1_ref*max_blocks;

    hipMalloc((void **)&(fields_cuda.tem_dev), allblocks_size_f1);
    hipMemset(fields_cuda.tem_dev, 0., allblocks_size_f1);

    hipMalloc((void **)&(fields_cuda.rhs_tem_dev), allblocks_size_f1);
    hipMemset(fields_cuda.rhs_tem_dev, 0., allblocks_size_f1);

    hipMalloc((void **)&(fields_cuda.prhs_tem_dev), allblocks_size_f1);
    hipMemset(fields_cuda.prhs_tem_dev, 0., allblocks_size_f1);

    hipMalloc((void **)&(fields_cuda.dist_dev), allblocks_size_f1);
    hipMemset(fields_cuda.dist_dev, 0., allblocks_size_f1);

    for(auto &t_dev: fields_cuda.temp_dev) {
        hipMalloc((void **)&(t_dev), allblocks_size_f1);
        hipMemset(t_dev, 0., allblocks_size_f1);
    }

    hipMalloc((void **)&(fields_cuda.work_ref_dev), allblocks_size_f1_ref);
    hipMemset(fields_cuda.work_ref_dev, 0., allblocks_size_f1_ref);
    ...
    hipMalloc((void **)&(fields_cuda.i_n_1_dev), n_size);
    hipMalloc((void **)&(fields_cuda.i_n_2_dev), n_size);
    hipMalloc((void **)&(fields_cuda.i_n_3_dev), n_size);
    hipMalloc((void **)&(fields_cuda.i_n_4_dev), n_size);
    hipMalloc((void **)&(fields_cuda.n_type_dev), n_size);
    hipMalloc((void **)&(fields_cuda.coord_x_rel_dev), n_size);
    hipMalloc((void **)&(fields_cuda.coord_y_rel_dev), n_size);
    hipMalloc((void **)&(fields_cuda.coord_z_rel_dev), n_size);

    // start cuda streams
    for (int i = 0; i < 9; i++)
        hipStreamCreate(&(fields_cuda.streams[i]));
...
void update_cpu_gpu(std::unordered_map<std::string, Field> & field_map, Fields_cuda & fields_cuda, 
    ...
    for(auto &n: field_map) {
        ...
        if(dir == "cpu_to_gpu") {
            hipMemcpy(T_tem_dev,      &(field.tem[0][0][0]),      sf1b, hipMemcpyHostToDevice);
            hipMemcpy(T_rhs_tem_dev,  &(field.rhs_tem[0][0][0]),  sf1b, hipMemcpyHostToDevice);
            hipMemcpy(T_prhs_tem_dev, &(field.prhs_tem[0][0][0]), sf1b, hipMemcpyHostToDevice);
            hipMemcpy(T_dist_dev,     &(field.dist[0][0][0]),     sf1b, hipMemcpyHostToDevice);
            hipMemcpy(T_dx_dev,       &(field.geo.lengths[0][3]), sf,   hipMemcpyHostToDevice);
            hipMemcpy(T_dy_dev,       &(field.geo.lengths[1][3]), sf,   hipMemcpyHostToDevice);
            hipMemcpy(T_dz_dev,       &(field.geo.lengths[2][3]), sf,   hipMemcpyHostToDevice);
            hipMemcpy(T_level_dev,    &level,                     sf,   hipMemcpyHostToDevice);
            ...
            hipMemcpy(&(field.tem[0][0][0]),      T_tem_dev,      sf1b, hipMemcpyDeviceToHost);
            hipMemcpy(&(field.rhs_tem[0][0][0]),  T_rhs_tem_dev,  sf1b, hipMemcpyDeviceToHost);
            hipMemcpy(&(field.prhs_tem[0][0][0]), T_prhs_tem_dev, sf1b, hipMemcpyDeviceToHost);
            hipMemcpy(&(field.dist[0][0][0]),     T_dist_dev,     sf1b, hipMemcpyDeviceToHost);
        }
        T_tem_dev      += sf1;
        T_rhs_tem_dev  += sf1;
        T_prhs_tem_dev += sf1;
    ...
    if(dir == "cpu_to_gpu") {
        ...
        hipMemcpy(fields_cuda.i_n_1_dev, fields_cuda.i_n_1, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.i_n_2_dev, fields_cuda.i_n_2, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.i_n_3_dev, fields_cuda.i_n_3, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.i_n_4_dev, fields_cuda.i_n_4, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.n_type_dev, fields_cuda.n_type, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.coord_x_rel_dev, fields_cuda.coord_x_rel, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.coord_y_rel_dev, fields_cuda.coord_y_rel, sf1c, hipMemcpyHostToDevice);
        hipMemcpy(fields_cuda.coord_z_rel_dev, fields_cuda.coord_z_rel, sf1c, hipMemcpyHostToDevice);
    }
}
...
__global__ void prhs_compute_dev_kernel(float *tem_dev, float *prhs_tem_dev, float *rhs_tem_dev, float qnrk, float * dist_dev, 
    ...
    int k  = threadIdx.x+blockIdx.x*blockDim.x; // coalescence
    ...
    if(j >= ngy && j < (ngy+ny) && k >= ngz && k < (ngz+nz) && ib < n_blocks) {
...
void prhs_compute_dev(Fields_cuda & fields_cuda, Params & params) {
    ...
    dim3 cuda_blocks = {16, 8, 8};
    unsigned int grid_x = (nz+2*ngz+cuda_blocks.x-1)/cuda_blocks.x;
    unsigned int grid_y = (ny+2*ngy+cuda_blocks.y-1)/cuda_blocks.y;
    ...
    hipLaunchKernelGGL((prhs_compute_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, fields_cuda.tem_dev, fields_cuda.prhs_tem_dev, 
...
void linear_compute_dev(Fields_cuda & fields_cuda, Params & params) {
    ...
    hipLaunchKernelGGL((linear_compute_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, fields_cuda.tem_dev, fields_cuda.prhs_tem_dev, 
        fields_cuda.rhs_tem_dev, qnrk, fields_cuda.dist_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);
    hipDeviceSynchronize();
...
void rhs_compute_dev(Fields_cuda & fields_cuda, Params & params) {
    ...
    hipLaunchKernelGGL((rhs_compute_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, fields_cuda.tem_dev, fields_cuda.rhs_tem_dev, 
...
void evolve_advanced_immerse_dev(Fields_cuda & fields_cuda, Params & params) {
    ...
    hipLaunchKernelGGL((evolve_advanced_immerse_dev_kernel_p1), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, fields_cuda.tem_dev,
        fields_cuda.temp_dev[0], fields_cuda.dist_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev,
        nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);

    hipLaunchKernelGGL((evolve_advanced_immerse_dev_kernel_p2), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, fields_cuda.tem_dev,
        fields_cuda.temp_dev[0], fields_cuda.dist_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev,
        nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);
    ...
    if(j >= ngy && j < (ngy+ny) && k >= ngz && k < (ngz+nz) && ib < n_blocks) {
        //for(ib=0;ib<n_blocks;ib++) {
            for(i=ngx;i<ngx+nx;i++) {
                ind = k+j*(nz+2*ngz)+i*(nz+2*ngz)*(ny+2*ngy)+ib*f1_size;
                if(dist_dev[ind] < 0.) {
                    f_dev_new[ind] = 2.*imposed_val-f_dev[ind];
                }
            }
       // }
        //for(ib=0;ib<n_blocks;ib++) {
...
void correct_advanced_immerse_dev(Fields_cuda & fields_cuda, Params & params) {
    ...
    hipLaunchKernelGGL((correct_advanced_immerse_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, 0, imposed_val, fields_cuda.tem_dev,
        fields_cuda.temp_dev[0], fields_cuda.dist_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev,
        nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);
    hipDeviceSynchronize();
...
__global__ void update_bc_phys_same_lr_dev_kernel(float *f_dev, float *dx_dev, float *dy_dev, float *dz_dev, 
    ...
    int ib, ib_n_1, i;
    size_t ind, ind_p;
    if(j >= ngy && j < (ngy+ny) && k >= ngz && k < (ngz+nz)) {
        for(ib=0; ib < n_blocks; ib++) {
...
__global__ void update_bc_phys_same_bt_dev_kernel(float *f_dev, float *dx_dev, float *dy_dev, float *dz_dev, 
    ...
    int ib, ib_n_1, j;
    size_t ind, ind_p;
    if(i >= ngx && i < (ngx+nx) && k >= ngz && k < (ngz+nz)) {
        for(ib=0; ib < n_blocks; ib++) {
...
__global__ void update_bc_phys_same_bf_dev_kernel(float *f_dev, float *dx_dev, float *dy_dev, float *dz_dev, 
    ...
    int j = threadIdx.x+blockIdx.x*blockDim.x; // coalescence
    int i = threadIdx.y+blockIdx.y*blockDim.y;
    int ib, ib_n_1, k;
    size_t ind, ind_p;
    if(j >= ngy && j < (ngy+ny) && i >= ngx && i < (ngx+nx)) {
        for(ib=0; ib < n_blocks; ib++) {
...
void update_bc_dev(std::unordered_map<std::string,Geometry> &geo_map, 
    ...
    // phys and same
    cuda_blocks = {32, 16, 1};
    grid_x = (nz+2*ngz+cuda_blocks.x-1)/cuda_blocks.x;
    grid_y = (ny+2*ngy+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_phys_same_lr_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[0], fields_cuda.tem_dev, 
    ...
    cuda_blocks = {32, 16, 1};
    grid_x = (nz+2*ngz+cuda_blocks.x-1)/cuda_blocks.x;
    grid_y = (nx+2*ngx+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_phys_same_bt_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[1], fields_cuda.tem_dev, 
        fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, fields_cuda.i_n_1_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);

    cuda_blocks = {32, 16, 1};
    grid_x = (ny+2*ngy+cuda_blocks.x-1)/cuda_blocks.x;
    grid_y = (nx+2*ngx+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_phys_same_bf_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[2], fields_cuda.tem_dev, 
        fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, fields_cuda.i_n_1_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size);
    ...
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_less_lr_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[3], fields_cuda.tem_dev, 
        fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, fields_cuda.i_n_1_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size,
    ...
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_less_bt_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[4], fields_cuda.tem_dev, 
        fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, fields_cuda.i_n_1_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size,
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    grid_y = (nx/2+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_less_bf_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[5], fields_cuda.tem_dev, 
        fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, fields_cuda.i_n_1_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size,
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p1_lr_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[6], fields_cuda.tem_dev, 
    ...
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p1_bt_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[7], fields_cuda.tem_dev, 
        fields_cuda.work_ref_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, 
        fields_cuda.i_n_1_dev, fields_cuda.i_n_2_dev, fields_cuda.i_n_3_dev, fields_cuda.i_n_4_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size, f1_size_ref, 
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    grid_y = (nx+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p1_bf_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[8], fields_cuda.tem_dev, 
        fields_cuda.work_ref_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, 
        fields_cuda.i_n_1_dev, fields_cuda.i_n_2_dev, fields_cuda.i_n_3_dev, fields_cuda.i_n_4_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size, f1_size_ref,
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    grid_y = (ny+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p2_lr_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[6], fields_cuda.tem_dev, 
        fields_cuda.work_ref_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, 
        fields_cuda.i_n_1_dev, fields_cuda.i_n_2_dev, fields_cuda.i_n_3_dev, fields_cuda.i_n_4_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size, f1_size_ref, 
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    grid_y = (nx+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p2_bt_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[7], fields_cuda.tem_dev, 
        fields_cuda.work_ref_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, 
        fields_cuda.i_n_1_dev, fields_cuda.i_n_2_dev, fields_cuda.i_n_3_dev, fields_cuda.i_n_4_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size, f1_size_ref, 
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
    ...
    grid_y = (nx+cuda_blocks.y-1)/cuda_blocks.y;
    cuda_grid  = {grid_x, grid_y, 1};
    hipLaunchKernelGGL((update_bc_more_p2_bf_dev_kernel), dim3(cuda_grid), dim3(cuda_blocks), 0, fields_cuda.streams[8], fields_cuda.tem_dev, 
        fields_cuda.work_ref_dev, fields_cuda.dx_dev, fields_cuda.dy_dev, fields_cuda.dz_dev, 
        fields_cuda.i_n_1_dev, fields_cuda.i_n_2_dev, fields_cuda.i_n_3_dev, fields_cuda.i_n_4_dev, 
        fields_cuda.n_type_dev, nx, ny, nz, ngx, ngy, ngz, n_blocks, f1_size, f1_size_ref, 
        fields_cuda.coord_x_rel_dev, fields_cuda.coord_y_rel_dev, fields_cuda.coord_z_rel_dev);
