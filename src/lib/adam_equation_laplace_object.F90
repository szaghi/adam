...
contains
   ...
   class(field_object),            intent(inout)        :: field          !< The field.
   ...
   do b=1, field%blocks_number
      ...
      associate (ni=>field%grid%ni, nj=>field%grid%nj, nk=>field%grid%nk)
         do k=1, nk
            do j=1, nj
               do i=1, ni
                  grad_q = max(grad_q, sqrt(((field%q(i+1,j,k,b) - field%q(i-1,j,k,b))/(2*field%dxyz(1,b)))**2 + &
                                            ((field%q(i,j+1,k,b) - field%q(i,j-1,k,b))/(2*field%dxyz(2,b)))**2 + &
                                            ((field%q(i,j,k+1,b) - field%q(i,j,k-1,b))/(2*field%dxyz(3,b)))**2))
   ...
   do s=1, 3
      ...
      if (do_ghost_syncro_) then
         ...
         call self%compute_residuals(field=field,q=q_s(:,:,:,:,s), t=t + gamm(s) * Dt, block_start=1, block_end=inner_blocks_number)
   ...
   class(equation_laplace_object), intent(in)    :: self                    !< The equation.
   class(field_object),            intent(in)    :: field                   !< The field.
   real(R8P),                      intent(inout) :: q(1-field%grid%gci:,&
                                                      1-field%grid%gcj:,&
                                                      1-field%grid%gck:,1:) !< Field component to be updated.
   real(R8P),                      intent(in)    :: t                       !< Time.
      ...
      do k=1, field%grid%nk
         do j=1, field%grid%nj
            do i=1, field%grid%ni
               ...
               q_work(i,j,k) = (q(i+1,j,k,b) - q(i-1,j,k,b))/(2*field%dxyz(1,b))
            enddo
         enddo
      enddo
      q(1:field%grid%ni,1:field%grid%nj,1:field%grid%nk,b) = q_work(1:field%grid%ni,1:field%grid%nj,1:field%grid%nk)
   enddo
         ...
         elseif (bc_type == BC_INFLOW) then
            do k=ijkmin(3), ijkmax(3), sign(1, ijkmax(3)-ijkmin(3))
               do j=ijkmin(2), ijkmax(2), sign(1, ijkmax(2)-ijkmin(2))
                  do i=ijkmin(1), ijkmax(1), sign(1, ijkmax(1)-ijkmin(1))
                     ! q(i,j,k,b) = 1._R8P
                     q(i,j,k,b) = exp(-((field%y_cell(j,b) - 0.5)**2/(2 * 0.2**2)+&
