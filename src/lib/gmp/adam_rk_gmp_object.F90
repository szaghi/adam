...
module adam_rk_gmp_object
!< ADAM, RK class GMP (GMP backend of [[rk_object]]).

use adam_rk_object
use adam_memory_gmp_library
use adam_mpih_gmp_object
use adam_rk_gmp_kernels
use penf

implicit none
save
private
public :: rk_gmp_object
...
endtype rk_gmp_object
contains
   ! public methods
   subroutine assign_stage(self, s, q_gpu, phi_gpu)
   !< Assign q to RK stage.
   class(rk_gmp_object), intent(inout)        :: self        !< RK object.
   ...
   associate(ni=>self%rk%ni, nj=>self%rk%nj, nk=>self%rk%nk, ngc=>self%rk%ngc, nv=>self%rk%nv, blocks_number=>self%rk%blocks_number)
      call rk_assign_stage_gmp(ni=ni,nj=nj,nk=nk,ngc=ngc,nv=nv,blocks_number=blocks_number,s=s,phi_gpu=phi_gpu,q_gpu=q_gpu, &
   ...
   associate(ni=>self%rk%ni, nj=>self%rk%nj, nk=>self%rk%nk, ngc=>self%rk%ngc, nv=>self%rk%nv, blocks_number=>self%rk%blocks_number)
      call rk_compute_stage_gmp(ni=ni, nj=nj, nk=nk, ngc=ngc, nv=nv, blocks_number=blocks_number, s=s, dt=dt, alph=self%alph_gpu, &
   ...
   class(rk_gmp_object), intent(in)           :: self        !< RK object.
   ...
   associate(ni=>self%rk%ni, nj=>self%rk%nj, nk=>self%rk%nk, ngc=>self%rk%ngc, nv=>self%rk%nv, blocks_number=>self%rk%blocks_number)
      call rk_compute_stage_ls_gmp(ni=ni, nj=nj, nk=nk, ngc=ngc, nv=nv, blocks_number=blocks_number, dt=dt, &
   ...
   !< Initialize class.
   class(rk_gmp_object),  intent(inout)      :: self !< RK GMP object.
   ...
   self%mpih => mpih
   call self%mpih%print_message('rk_gmp_object%initialize start')
   self%rk => rk
   msg_ = self%mpih%myrankstr//'rk_gmp_object%initialize'
   call assign_allocatable_gpu(lhs=self%alph_gpu, rhs=rk%alph, omp_dev=self%mpih%mydev, msg=msg_//' rk_alph ')
   call assign_allocatable_gpu(lhs=self%beta_gpu, rhs=rk%beta, omp_dev=self%mpih%mydev, msg=msg_//' rk_beta ')
   call assign_allocatable_gpu(lhs=self%gamm_gpu, rhs=rk%gamm, omp_dev=self%mpih%mydev, msg=msg_//' rk_gamm ')
   msg = msg_//' q_rk_gpu '
   ...
   associate(ni=>self%rk%ni, nj=>self%rk%nj, nk=>self%rk%nk, ngc=>self%rk%ngc, nv=>self%rk%nv, blocks_number=>self%rk%blocks_number)
      call rk_initialize_stages_gmp(ni=ni,nj=nj,nk=nk,ngc=ngc,nv=nv,blocks_number=blocks_number,q_gpu=q_gpu,q_rk_gpu=self%q_rk_gpu)
   endassociate
   ...
   associate(ni=>self%rk%ni, nj=>self%rk%nj, nk=>self%rk%nk, ngc=>self%rk%ngc, nv=>self%rk%nv, blocks_number=>self%rk%blocks_number)
      call rk_update_q_gmp(ni=ni,nj=nj,nk=nk,ngc=ngc,nv=nv,blocks_number=blocks_number,nrk=self%rk%nrk,dt=dt,beta=self%beta_gpu, &
                           phi_gpu=phi_gpu,q_rk_gpu=self%q_rk_gpu,q_gpu=q_gpu)
   endassociate
   endsubroutine update_q
endmodule adam_rk_gmp_object
