...
contains
   ...
   if (omp_target_is_present_f(var,omp_dev)) then
      call omp_target_free_f(var,omp_dev)
   endif
   !if (verbose_) then
   ...
   if (verbose_) then
      if (error/=0) then
         print '(A)', msg_//'Error allocating variable'
      else
         print '(A)', msg_//'Variable allocation OK!'
      endif
   endif
   ...
   if (omp_target_is_present_f(lhs,omp_dev)) then
      call omp_target_free_f(lhs,omp_dev)
   endif
   if (allocated(rhs)) then
      if (size(rhs, dim=1)>0) then
         call alloc_var_gpu(var=lhs, ulb=[lbound(rhs,dim=1),ubound(rhs,dim=1)], omp_dev=omp_dev, msg=msg, verbose=verbose)
   ...
   if (allocated(rhs)) then
      if (size(rhs, dim=1)*size(rhs, dim=2)>0) then
         ...
         if (transposed_) then
            ...
            call alloc_var_gpu(var=lhs, ulb=reshape([ulb(1,2),ulb(2,2),ulb(1,1),ulb(2,1)],[2,2]), omp_dev=omp_dev, msg=msg, verbose=verbose)
            ...
            call alloc_var_gpu(var=lhs, ulb=ulb, msg=msg, omp_dev=omp_dev, verbose=verbose)
            error = omp_target_memcpy_f(fptr_dst=lhs, fptr_src=rhs, dst_off=0_I4P, src_off=0_I4P, &
                                        omp_dst_dev=omp_dev, omp_src_dev=int(omp_get_initial_device(),I4P))
         endif
      endif
   ...
   if (allocated(rhs)) then
      if (size(rhs, dim=1)*size(rhs, dim=2)>0) then
         call alloc_var_gpu(var=lhs,                                                                                     &
                            ulb=reshape([lbound(rhs,dim=1),ubound(rhs,dim=1),lbound(rhs,dim=2),ubound(rhs,dim=2)],[2,2]),&
                            omp_dev=omp_dev, msg=msg, verbose=verbose)
         error = omp_target_memcpy_f(fptr_dst=lhs, fptr_src=rhs, dst_off=0_I4P, src_off=0_I4P, &
                                     omp_dst_dev=omp_dev, omp_src_dev=int(omp_get_initial_device(),I4P))
      endif
   endif
