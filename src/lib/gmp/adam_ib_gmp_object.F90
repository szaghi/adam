...
type :: ib_gmp_object
   ...
   ! device data
   real(R8P), pointer, contiguous :: q_bcs_vars_gpu(:,:)=>null() !< Variables array for immersed boundary on GPU.
   real(R8P), pointer, contiguous :: phi_gpu(:,:,:,:,:)=>null()  !< Distance function on GPU.
