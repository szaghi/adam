...
type :: field_object
   ...
   contains
      ...
      procedure, pass(self) :: set_comm_local_ghost          !< Set communication and local maps/buffers for ghosts update.
      procedure, pass(self) :: set_local_bc                  !< Set local maps for boundary conditions.
      ! private methods
...
contains
   ...
   subroutine set_comm_local_ghost(self,                    &
                                   local_map_ghost,         &
                                   local_map_ghost_cell,    &
                                   comm_map_n_send_ghost,   &
                                   comm_map_n_recv_ghost,   &
                                   comm_map_send_ptr_ghost, &
                                   comm_map_recv_ptr_ghost, &
                                   comm_map_send_ghost,     &
                                   comm_map_send_ghost_cell,&
                                   comm_map_recv_ghost,     &
                                   comm_map_recv_ghost_cell,&
                                   verbose)
   !< Set communication and local maps for ghosts update and send/receive buffer.
   class(field_object),       intent(inout)        :: self                          !< The field.
   integer(I8P), allocatable, intent(in)           :: local_map_ghost(:,:)          !< Local map for ghost cells updating [1:,1:].
   integer(I8P), allocatable, intent(in)           :: local_map_ghost_cell(:,:)     !< Local map ghost cells update, cells order.
   ...
   integer(I8P), allocatable, intent(in)           :: comm_map_send_ghost(:,:)      !< Comm map, send ghost cells, fec order.
   integer(I8P), allocatable, intent(in)           :: comm_map_send_ghost_cell(:,:) !< Comm map, send ghost cells, cells order.
   integer(I8P), allocatable, intent(in)           :: comm_map_recv_ghost(:,:)      !< Comm map, recv ghost cells, fec order.
   integer(I8P), allocatable, intent(in)           :: comm_map_recv_ghost_cell(:,:) !< Comm map, recv ghost cells, cells order.
   logical,                   intent(in), optional :: verbose                       !< Flag to activate verbose mode.

   call assign_allocatable_cpu(lhs=self%local_map_ghost, &
                               rhs=     local_map_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(local_map_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%local_map_ghost_cell, &
                               rhs=     local_map_ghost_cell, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(local_map_ghost_cell) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_n_send_ghost, &
                               rhs=     comm_map_n_send_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_n_send_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_n_recv_ghost, &
                               rhs=     comm_map_n_recv_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_n_recv_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_send_ptr_ghost, &
                               rhs=     comm_map_send_ptr_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_send_ptr_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_recv_ptr_ghost, &
                               rhs=     comm_map_recv_ptr_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_recv_ptr_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_send_ghost, &
                               rhs=     comm_map_send_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_send_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_send_ghost_cell, &
                               rhs=     comm_map_send_ghost_cell, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_send_ghost_cell) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_recv_ghost, &
                               rhs=     comm_map_recv_ghost, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_recv_ghost) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%comm_map_recv_ghost_cell, &
                               rhs=     comm_map_recv_ghost_cell, &
                               msg=self%mpih%myrankstr//'field%set_comm_local_ghost(comm_map_recv_ghost_cell) ', verbose=verbose)
   ...
   if (allocated(self%comm_map_n_send_ghost)) then
      call alloc_var_cpu(var=self%send_buffer_ghost,                              &
                         ...
                         msg=self%mpih%myrankstr//'field%set_comm_local_ghost(send_buffer_ghost) ', verbose=verbose)
   endif
   ...
   if (allocated(self%comm_map_n_recv_ghost)) then
      call alloc_var_cpu(var=self%recv_buffer_ghost,                              &
                         ...
                         msg=self%mpih%myrankstr//'field%set_comm_local_ghost(recv_buffer_ghost) ', verbose=verbose)
   endif
   endsubroutine set_comm_local_ghost

   subroutine set_local_bc(self, local_map_bc_face, local_map_bc_edge, local_map_bc_corner, local_map_bc_crown, verbose)
   !< Set local maps of boundary conditions.
   class(field_object),       intent(inout)        :: self                      !< The field.
   ...
   integer(I8P), allocatable, intent(in)           :: local_map_bc_crown(:,:,:) !< Local map for crown BC ghost cells.
   logical,                   intent(in), optional :: verbose                   !< Flag to activate verbose mode.

   call assign_allocatable_cpu(lhs=self%local_map_bc_face, &
                               rhs=     local_map_bc_face, &
                               msg=self%mpih%myrankstr//'field%set_local_bc(local_map_bc_face) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%local_map_bc_edge, &
                               rhs=     local_map_bc_edge, &
                               msg=self%mpih%myrankstr//'field%set_local_bc(local_map_bc_edge) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%local_map_bc_corner, &
                               rhs=     local_map_bc_corner, &
                               msg=self%mpih%myrankstr//'field%set_local_bc(local_map_bc_corner) ', verbose=verbose)
   call assign_allocatable_cpu(lhs=self%local_map_bc_crown, &
                               rhs=     local_map_bc_crown, &
                               msg=self%mpih%myrankstr//'field%set_local_bc(local_map_bc_crown) ', verbose=verbose)
   endsubroutine set_local_bc
