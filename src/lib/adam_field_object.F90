...
contains
   ...
   subroutine initialize(self, grid, tree, file_parameters, nv, nb)
   !< Initialize field.
   class(field_object), intent(inout)           :: self            !< The field.
   ...
   print '(A)', self%mpih%myrankstr//'field_cpu_object%initialize start'
   self%grid => grid
   self%tree => tree
   ...
   if (self%nb>0) then
      ...
      call alloc_var_cpu(var=self%q,                                              &
                         ulb=reshape([1,self%nv,                                  &
                                      ...
                                      1,self%nb],[2,5]),                          &
                         msg=self%mpih%myrankstr//'field_cpu_object%initialize(q) ', verbose=.true.)
      call alloc_var_cpu(var=self%q_work,                                         &
                         ulb=reshape([1,self%nv,                                  &
                                      ...
                                      1-self%grid%ngc,self%grid%nk+self%grid%ngc, &
                                      1,self%nb],[2,5]),                          &
                         msg=self%mpih%myrankstr//'field_cpu_object%initialize(q_work) ', verbose=.true.)
      self%q = 0._R8P
      self%q_work = 0._R8P
   endif
   call alloc_var_cpu(var=self%blocks_numbers, &
                      ulb=[0,self%mpih%procs_number-1],  &
                      msg=self%mpih%myrankstr//'field_cpu_object%initialize(blocks_numbers) ', verbose=.true.)
   call alloc_var_cpu(var=self%req_send_recv,  &
                      ulb=[0,self%mpih%procs_number*2-1],&
                      msg=self%mpih%myrankstr//'field_cpu_object%initialize(req_send_recv) ', verbose=.true.)
   print '(A)', self%description()
   print '(A)', self%mpih%myrankstr//'field_cpu_object%initialize finish'
   ...
   !<
   !< @note The field `q` to be updated is passed and not taken from `self` thus `q` could be any conformable array,
   !<  not necessary the `self%q`.
   class(field_object), intent(in)    :: self                   !< The field.
   real(R8P),           intent(inout) :: q(1:,              &
                                           1-self%grid%ngc:,&
                                           1-self%grid%ngc:,&
                                           1-self%grid%ngc:,1:) !< Field component to be updated.
