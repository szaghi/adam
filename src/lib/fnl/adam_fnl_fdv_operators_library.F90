...
contains
   ...
   ! centered
   pure subroutine compute_curl_fv_centered_dev(s,dxyz,qsx_y,qsx_z,qsy_x,qsy_z,qsz_x,qsz_y,curl)
   !< Compute curl of q vector field with finite volume centered scheme, device backend.
   !< Gathered-buffer signature (mirrors compute_curl_fd_centered_dev): the caller pre-gathers the six
   !< contiguous directional stencils; this routine flux-difference-reconstructs the six derivatives via
   !< the rebuilt FV centered d1, avoiding non-contiguous sections inside acc routine seq.
   integer(I4P), intent(in)  :: s              !< Stencil len, half of accuracy order.
   real(R8P),    intent(in)  :: dxyz(1:)       !< Space steps [1:3].
   real(R8P),    intent(in)  :: qsx_y(1-s:)    !< Y component of vector field over the x stencil.
   real(R8P),    intent(in)  :: qsx_z(1-s:)    !< Z component of vector field over the x stencil.
   ...
   call compute_derivative1_fv_centered(s=s,ds=dxyz(2),q=qsy_x(1-s:1+s),dq_ds=dqx_dy)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(3),q=qsz_x(1-s:1+s),dq_ds=dqx_dz)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(1),q=qsx_y(1-s:1+s),dq_ds=dqy_dx)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(3),q=qsz_y(1-s:1+s),dq_ds=dqy_dz)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(1),q=qsx_z(1-s:1+s),dq_ds=dqz_dx)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(2),q=qsy_z(1-s:1+s),dq_ds=dqz_dy)
   curl(1) = dqz_dy - dqy_dz
   curl(2) = dqx_dz - dqz_dx
   curl(3) = dqy_dx - dqx_dy
   endsubroutine compute_curl_fv_centered_dev

   pure subroutine compute_divergence_fv_centered_dev(s,dxyz,qsx,qsy,qsz,divergence)
   !< Compute divergence of q vector field with finite volume centered scheme, device backend.
   !< Gathered-buffer signature: caller pre-gathers the three directional component stencils.
   integer(I4P), intent(in)  :: s                   !< Stencil len, half of accuracy order.
   real(R8P),    intent(in)  :: dxyz(1:)            !< Space steps [1:3].
   real(R8P),    intent(in)  :: qsx(1-s:)           !< X component of vector field over the x stencil.
   real(R8P),    intent(in)  :: qsy(1-s:)           !< Y component of vector field over the y stencil.
   ...
   call compute_derivative1_fv_centered(s=s,ds=dxyz(1),q=qsx(1-s:1+s),dq_ds=div_x)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(2),q=qsy(1-s:1+s),dq_ds=div_y)
   call compute_derivative1_fv_centered(s=s,ds=dxyz(3),q=qsz(1-s:1+s),dq_ds=div_z)
   divergence = div_x + div_y + div_z
   endsubroutine compute_divergence_fv_centered_dev

   pure subroutine compute_gradient_fv_centered_dev(s,dxyz,qsx,qsy,qsz,gradient)
   !< Compute gradient of q scalar field with finite volume centered scheme, device backend.
   !< Gathered-buffer signature: caller pre-gathers the three directional stencils of the scalar field.
   integer(I4P), intent(in)  :: s            !< Stencil len, half of accuracy order.
   real(R8P),    intent(in)  :: dxyz(1:)     !< Space steps [1:3].
   real(R8P),    intent(in)  :: qsx(1-s:)    !< Scalar field over the x stencil.
   real(R8P),    intent(in)  :: qsy(1-s:)    !< Scalar field over the y stencil.
   real(R8P),    intent(in)  :: qsz(1-s:)    !< Scalar field over the z stencil.
   real(R8P),    intent(out) :: gradient(1:) !< Gradient of q [1:3].
   !$acc routine seq
   !$omp declare target

   call compute_derivative1_fv_centered(s=s,ds=dxyz(1),q=qsx(1-s:1+s),dq_ds=gradient(1))
   call compute_derivative1_fv_centered(s=s,ds=dxyz(2),q=qsy(1-s:1+s),dq_ds=gradient(2))
   call compute_derivative1_fv_centered(s=s,ds=dxyz(3),q=qsz(1-s:1+s),dq_ds=gradient(3))
   endsubroutine compute_gradient_fv_centered_dev

   pure subroutine compute_laplacian_fv_centered_dev(s,dxyz,qsx,qsy,qsz,laplacian)
   !< Compute laplacian of q scalar field with finite volume centered scheme, device backend.
   !< Gathered-buffer signature: caller pre-gathers the three directional stencils; uses the rebuilt FV d2.
   integer(I4P), intent(in)  :: s                       !< Stencil len, half of accuracy order.
   real(R8P),    intent(in)  :: dxyz(1:)                !< Space steps [1:3].
   real(R8P),    intent(in)  :: qsx(1-s:)               !< Scalar field over the x stencil.
   real(R8P),    intent(in)  :: qsy(1-s:)               !< Scalar field over the y stencil.
   ...
   call compute_derivative2_fv_centered(s=s,ds=dxyz(1),q=qsx(1-s:1+s),d2q_ds2=d2q_dx2)
   call compute_derivative2_fv_centered(s=s,ds=dxyz(2),q=qsy(1-s:1+s),d2q_ds2=d2q_dy2)
   call compute_derivative2_fv_centered(s=s,ds=dxyz(3),q=qsz(1-s:1+s),d2q_ds2=d2q_dz2)
   laplacian = d2q_dx2 + d2q_dy2 + d2q_dz2
