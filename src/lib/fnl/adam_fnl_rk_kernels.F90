...
contains
   ...
   if (present(phi_gpu)) then
      all_solids = ubound(phi_gpu, dim=5)
      !$acc parallel loop independent DEVICEVAR(phi_gpu, alph, q_rk_gpu)
      !$omp OMPLOOP DEVICEVAR(phi_gpu, alph, q_rk_gpu)
      do ss=1, s-1
         do v=1, nv
            do k=1, nk
               do j=1, nj
      ...
      !$acc parallel loop independent DEVICEVAR(alph, q_rk_gpu)
      !$omp OMPLOOP DEVICEVAR(alph, q_rk_gpu)
      do ss=1, s-1
         do v=1, nv
            do k=1, nk
               do j=1, nj
   ...
   if (present(phi_gpu)) then
      all_solids = ubound(phi_gpu, dim=5)
      !$acc parallel loop independent DEVICEVAR(phi_gpu, beta, q_rk_gpu, q_gpu)
      !$omp OMPLOOP DEVICEVAR(phi_gpu, beta, q_rk_gpu, q_gpu)
      do s=1, nrk
         do v=1, nv
            do k=1, nk
               do j=1, nj
      ...
      !$acc parallel loop independent DEVICEVAR(beta, q_rk_gpu, q_gpu)
      !$omp OMPLOOP DEVICEVAR(beta, q_rk_gpu, q_gpu)
      do s=1, nrk
         do v=1, nv
            do k=1, nk
               do j=1, nj
