...
contains
   ...
   !$omp OMPLOOP DEVICEPTR(dx_gpu, dy_gpu, dz_gpu, phi_gpu, q_gpu, dq_gpu)
   do k=1, nk
      do j=1, nj
         do i=1, ni
   ...
   !$omp OMPLOOP DEVICEPTR(x_cell_gpu, y_cell_gpu, z_cell_gpu, phi_gpu)
   do k=1, nk
      do j=1, nj
         do i=1, ni
            do b=1, blocks_number
   ...
   !$omp OMPLOOP DEVICEPTR(phi_gpu, dq_gpu, q_gpu)
   do k=1, nk
      do j=1, nj
         do i=1,ni
            do b=1, blocks_number
   ...
   if     (bcs_type == BCS_VISCOUS) then
      ...
      !$omp OMPLOOP DEVICEPTR(phi_gpu, q_gpu)
      do k=1-ngc, nk+ngc
         do j=1-ngc, nj+ngc
            do i=1-ngc, ni+ngc
               do b=1, blocks_number
   ...
   !$omp OMPLOOP DEVICEPTR(phi_gpu, dphi_gpu)
   do k=1, nk
   do j=1, nj
   do i=1, ni
   do b=1, blocks_number
   ...
   !$omp OMPLOOP DEVICEPTR(phi_gpu, cell_scheme_gpu)
   do b=1, blocks_number
      do j=1, nj
         do k=1, nk
            idir: do i=0, ni
