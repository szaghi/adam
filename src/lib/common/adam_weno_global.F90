...
!< `equation_object` as value components, so the true storage lives at
...
!< in equation_object%initialize.
!<
!< The shim deliberately does NOT `use adam_equation_object` — that would
...
!< cannot live in a separate module — a binder taking `class(equation_object)`
...
type(weno_object), pointer :: weno => null() !< Program-scope WENO shim, bound inside equation_object%initialize.
