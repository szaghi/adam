...
public :: move_phi_cuf
public :: reduce_cell_order_phi_cuf
...
contains
   ...
   real(R8P),    intent(in),    device :: q_gpu(  1:,1-ngc:,1-ngc:,1-ngc:,1:) !< State variables.
   real(R8P),    intent(inout), device :: dq_gpu( 1:,1-ngc:,1-ngc:,1-ngc:,1:) !< State variables variations.
   ...
   real(R8P),    intent(in),    device :: x_cell_gpu(1:,1-ngc:)                !< Cells x coordinates on GPU.
   real(R8P),    intent(in),    device :: y_cell_gpu(1:,1-ngc:)                !< Cells y coordinates on GPU.
   real(R8P),    intent(in),    device :: z_cell_gpu(1:,1-ngc:)                !< Cells z coordinates on GPU.
   real(R8P),    intent(inout), device :: phi_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:)  !< Distance function.
   ...
   enddo
   !@cuf iercuda=cudaDeviceSynchronize()
   endsubroutine compute_phi_analytical_sphere_cuf
   ...
   integer(I4P), intent(in)            :: blocks_number                       !< Number of blocks.
   real(R8P),    intent(in),    device :: phi_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Distance function.
   real(R8P),    intent(in),    device :: dq_gpu( 1:,1-ngc:,1-ngc:,1-ngc:,1:) !< State variables variation.
   real(R8P),    intent(inout), device :: q_gpu(  1:,1-ngc:,1-ngc:,1-ngc:,1:) !< State variables.
   integer(I4P)                        :: i, j, k, b, v                       !< Counter.
   integer(I4P)                        :: iercuda                             !< Error trapping flag for CUDAFortran.
   ...
   endsubroutine invert_eikonal_q_phi_cuf
   ...
   subroutine reduce_cell_order_phi_cuf(ib, ni, nj, nk, ngc, blocks_number, iweno, &
   ...
   real(R8P),    intent(in),    device :: phi_gpu(        1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Distance function.
   integer(I4P), intent(inout), device :: cell_scheme_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Modified order close to solids.
   ...
   enddo
   !@cuf iercuda=cudaDeviceSynchronize()
   endsubroutine reduce_cell_order_phi_cuf
endmodule adam_ib_nvf_kernels
