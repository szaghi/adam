...
use adam_mhd_common_object
use adam_mhd_schemes_object
use adam_mhd_nvf_kernels
use penf
use MPI
use CUDAFOR
...
implicit none
private
public :: mhd_nvf_object

type, extends(mhd_common_object) :: mhd_nvf_object
...
endtype mhd_nvf_object

contains
   ! auxiliary methods
   subroutine allocate_gpu(self)
   !< Allocate common data.
   class(mhd_nvf_object), intent(inout) :: self !< The equation.

   call self%mpih%print_message('mhd_nvf_object%allocate_gpu start')
   ...
   ! call alloc_var_gpu(var=self%q_gpu, msg=self%mpih%myrankstr//'equation_mhd_gpu%alloc(q_gpu) ', verbose=.false.,&
   ...
   endassociate
   call self%mpih%print_message('mhd_nvf_object%allocate_gpu finish')
   ...
   class(mhd_nvf_object), intent(inout) :: self  !< The equation.
   type(mhd_nvf_object)                 :: fresh !< Fresh equation.
   ...
   call self%mpih%print_message('mhd_nvf_object%initialize_gpu start')
   ...
   call self%mpih%print_message('mhd_nvf_object%initialize_gpu finish')
   ...
   class(mhd_nvf_object), intent(in)            :: self                  !< The equation.
