!< ADAM, MHD Equations Of State (EOS) class definition, CPU backend.
module adam_mhd_eos_object
!< ADAM, MHD Equations Of State (EOS) class definition, CPU backend.
...
type :: mhd_eos_object
   !< MHD Equations Of State (EOS, ideal, compressible fluid) class definition.
   ...
   contains
      ...
      ! operators
      generic :: assignment(=) => mhd_physics_assign_mhd_physics !< Overload `=`.
      ! private methods
      procedure, pass(lhs), private :: mhd_physics_assign_mhd_physics !< Operator `=`.
endtype mhd_eos_object

interface mhd_eos_object
   !< Overload [[mhd_eos_object]] name with its constructor.
   module procedure mhd_eos_instance
endinterface
contains
   ! public methods
   elemental subroutine compute_derivate(self)
   !< Compute derivate quantities (from `cp` and `cv`).
   class(mhd_eos_object), intent(inout) :: self !< Fluid physics.
   ...
   class(mhd_eos_object), intent(in) :: self            !< Fluid physics.
   ...
   !< Destroy physics.
   class(mhd_eos_object), intent(inout) :: self  !< Fluid physics.
   type(mhd_eos_object)                 :: fresh !< Fresh fluid physics.
   ...
   class(mhd_eos_object), intent(inout)        :: self    !< Fluid physics.
   type(mhd_eos_object),  intent(in), optional :: physics !< Fluid physics.
   ...
   if (present(physics)) then
      ...
      self = mhd_eos_object(mu=mu, kd=kd, dha=dha, cp=cp, cv=cv, gam=gam, R=R)
   endif
   ...
   !< Save into file.
   class(mhd_eos_object), intent(inout) :: self            !< Free conditions.
   ...
   ! operators
   pure subroutine mhd_physics_assign_mhd_physics(lhs, rhs)
   !< Operator `=`.
   class(mhd_eos_object), intent(inout) :: lhs !< Left hand side.
   type(mhd_eos_object),  intent(in)    :: rhs !< Right hand side.
   ...
   lhs%kd    = rhs%kd
   lhs%dha   = rhs%dha
   endsubroutine mhd_physics_assign_mhd_physics
   ...
   ! private non TBP
   elemental function mhd_eos_instance(mu, kd, dha, cp, cv, gam, R) result(instance)
   !< Return and instance of [[mhd_eos_object]].
   !<
   !< @note This procedure is used for overloading [[mhd_eos_object]] name.
   ...
   real(R8P), intent(in), optional :: gam      !< Specific heats ratio `gamma=cp/cv` value.
   real(R8P), intent(in), optional :: R        !< Fluid constant `R=cp-cv` value.
   type(mhd_eos_object)          :: instance !< Instance of [[mhd_eos_object]].
   ...
   endfunction mhd_eos_instance
endmodule adam_mhd_eos_object
