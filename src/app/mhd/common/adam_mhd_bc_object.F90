!< ADAM, MHD Boundary Conditions class definition, CPU backend.
module adam_mhd_bc_object
!< ADAM, MHD Boundary Conditions class definition, CPU backend.
...
endtype mhd_bc_object

contains
   ! public methods
   subroutine initialize(self, file_parameters)
   !< Initialize the equation.
   class(mhd_bc_object), intent(inout) :: self            !< BC.
   type(file_ini),         intent(in)    :: file_parameters !< Simulation parameters ini file handler.

   call self%mpih%initialize(do_mpi_init=.false.)
   print '(A)', self%mpih%myrankstr//'mhd_bc_object%initialize start'
   call self%load_from_file(file_parameters=file_parameters)
   print '(A)', self%mpih%myrankstr//'mhd_bc_object%initialize finish'
