...
implicit none
private
public :: mhd_io_object

character(len=2), parameter :: INI_SECTION_NAME="IO" !< INI (config) file section name containing IO configs.

type :: mhd_io_object
   !< MHD IO handler class definition, CPU backend.
   type(mpih_object)         :: mpih                       !< MPI handler.
   type(file_ini)            :: file_parameters            !< mhd input file handler.
...
endtype mhd_io_object

contains
   ! public methods
   pure function description(self) result(desc)
   !< Return a pretty-formatted object description.
   class(mhd_io_object), intent(in) :: self             !< IO handler.
   ...
   class(mhd_io_object), intent(inout) :: self     !< IO handler.
   character(*),           intent(in)    :: filename !< File name of parameters file.

   call self%mpih%initialize(do_mpi_init=.false.)
   print '(A)', self%mpih%myrankstr//'mhd_io_object%initialize start'
   ...
   print '(A)', self%mpih%myrankstr//'mhd_io_object%initialize finish'
