...
type :: prism_external_fields_object
	...
	real(R8P)		  :: RMF_phase = 0.0_R8P      !< Rotating magnetic field phase (degrees).
...
interface
    subroutine add_external_fields_interface(self, field, time, q)
...
contains 
	...
	desc = desc//NL//self%mpih%myrankstr//'    RMF phase (degrees): '//trim(str(self%RMF_phase))
    ...
    call file_parameters%get(section_name=INI_SECTION_NAME, option_name='RMF_phase', &
    val=self%RMF_phase, error=error)
    if (.not.go_on_fail_.and.error>0) & 
    call self%mpih%error_stop(msg=': failed to load ['//INI_SECTION_NAME//'].(RMF_phase)')
    ...
    subroutine add_external_fields_rmf(self, field, time, q)
	...
	real(R8P)										   :: theta, alfa														!< Angles in cylindrical coordinates
	...
	B_r = self%RMF_B_amplitude*cos(2*PI*self%RMF_frequency*time+self%RMF_phase/180.0_R8P*PI)
	B_theta = self%RMF_B_amplitude*sin(2*PI*self%RMF_frequency*time+self%RMF_phase/180.0_R8P*PI)
        ...
        do i = 1, ni
            do j = 1, nj
                do k = 1, nk
					...
					theta = abs(atan(cell_coord(beta)/cell_coord(alpha)))
					alfa = pi/2-theta
					if (cell_coord(alpha) > 0.0_R8P .and. cell_coord(beta) > 0.0_R8P) then ! 1 quadrante
						q(alpha+3_I4P,i,j,k,b) = q(alpha+3_I4P,i,j,k,b) + B_r*cos(theta) - B_theta*cos(alfa)
						q(beta+3_I4P,i,j,k,b)  = q(beta+3_I4P,i,j,k,b)  + B_r*sin(theta) + B_theta*sin(alfa)
						q(gamma,i,j,k,b) = sqrt(cell_coord(alpha)**2 + cell_coord(beta)**2)*2*PI*self%RMF_frequency* &
							self%RMF_B_amplitude*cos(2*PI*self%RMF_frequency*time+self%RMF_phase)*EPS0
    ...
    subroutine add_external_fields_none(self, field, time, q)
    !< Add rotating magnetic field to the field.
    class(prism_external_fields_object), intent(inout) :: self                                                              !< External fields.
    type(field_object),                  intent(inout) :: field                                                             !< The field.
    real(R8P),                           intent(in)    :: time                                                              !< Current simulation time.
