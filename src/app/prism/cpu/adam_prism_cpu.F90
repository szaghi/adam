...
!< matches Phase C behaviour. For N>1 (the rmf-2realm use case) the
!< shims alias `realm(N)` — that is the deferred C.3 gap documented in
!< [#10] Phase C status; the gap is fully load-bearing once N>1
!< actually runs and is the next Phase D follow-up after the
!< rmf-2realm regression case (D.4).
