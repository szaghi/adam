...
endtype nasto_eos_object

interface nasto_eos_object
   !< Overload [[nasto_eos_object]] name with its constructor.
   module procedure nasto_eos_instance
endinterface
contains
   ! public methods
   elemental subroutine compute_derivate(self)
   !< Compute derivate quantities (from `cp` and `cv`).
   class(nasto_eos_object), intent(inout) :: self !< Fluid physics.
   ...
   class(nasto_eos_object), intent(in) :: self            !< Fluid physics.
   ...
   !< Destroy physics.
   class(nasto_eos_object), intent(inout) :: self  !< Fluid physics.
   type(nasto_eos_object)                 :: fresh !< Fresh fluid physics.
   ...
   class(nasto_eos_object), intent(inout)        :: self    !< Fluid physics.
   type(nasto_eos_object),  intent(in), optional :: physics !< Fluid physics.
   ...
   if (present(physics)) then
      ...
      self = nasto_eos_object(mu=mu, kd=kd, dha=dha, cp=cp, cv=cv, gam=gam, R=R)
   endif
   ...
   !< Save into file.
   class(nasto_eos_object), intent(inout) :: self            !< Free conditions.
   ...
   !< Operator `=`.
   class(nasto_eos_object), intent(inout) :: lhs !< Left hand side.
   type(nasto_eos_object),  intent(in)    :: rhs !< Right hand side.
   ...
   ! private non TBP
   elemental function nasto_eos_instance(mu, kd, dha, cp, cv, gam, R) result(instance)
   ...
   real(R8P), intent(in), optional :: gam      !< Specific heats ratio `gamma=cp/cv` value.
   real(R8P), intent(in), optional :: R        !< Fluid constant `R=cp-cv` value.
   type(nasto_eos_object)          :: instance !< Instance of [[nasto_eos_object]].
   ...
   endfunction nasto_eos_instance
endmodule adam_nasto_eos_object
