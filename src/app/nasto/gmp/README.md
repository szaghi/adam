...
> ADAM for Navier-Stokes equations, GPU-OpenMP (GMP) backend.
...
of GPU accelerated backend, with MPI/OpenMP GPU offloading parallelization.

The main NASTO GMP documentation is contained in the following sections:
...
GPU accelerated architecture by means of MPI/OpenMP GPU offloading parallel pardigms.
...
┌╼ stefano@enlil
├───╼ ~/fortran/adam
└──────╼ tree src/app/nasto/gmp/
src/app/nasto/gmp/
├── adam_nasto.ini
├── adam_nasto_gmp.F90
├── adam_nasto_gmp_object.F90
└── README.md
```
...
┌╼ stefano@enlil
├───╼ ~/fortran/adam
└──────╼ FoBiS.py build -mode nasto-gmp-mpi-ifx
Builder options
  Directories
...
Building src/app/nasto/gmp/adam_nasto_gmp.F90
...
...
...
Linking exe/adam_nasto_gmp
Target src/app/nasto/gmp/adam_nasto_gmp.F90 has been successfully built
```
...
├───╼ ~/fortran/adam
└──────╼ ls exe/
adam_nasto_gmp  build_adam_nasto_gmp.log  mod  obj
```
...
Currently, NASTO GMP objects are made by the following source files:
...
This contains the definition of NASTO GMP object that extends the NASTO common object with all the necessary data and methods for the GMP backend.
See [nasto GMP object API documentantion](https://szaghi.github.io/adam/type/nasto_gmp_object.html) for more details.

### Main programs

#### `adam_nasto_gmp.F90`

This is only the main program that instantiates a `type(nasto_gmp_object)` object and invoke its `simulate` method.
See [nasto GMP program API documentantion](https://szaghi.github.io/adam/program/nasto_gmp.html) for more details.
