...
public :: compute_q_gradient_gmp
public :: compute_umax_gmp
public :: set_bc_q_gpu_gmp
! RK procedures
public :: compute_rk_q_gpu_gmp

contains
   ! numerical procedures
   subroutine compute_flux_conv_x_kernel(blocks_number, ni, nj, nk, ngc, nv, iweno, dha, g, R, cv,       &
                                         ror_threshold, enable_ror_stats, cell_scheme_gpu, ror_ivar_gpu, &
                                         ror_schemes_gpu, q_aux_gpu, ror_stats_gpu, flx_gpu)
   ...
   integer(I4P), intent(in)            :: cell_scheme_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:)
   integer(I4P), intent(in)            :: ror_ivar_gpu(1:)
   integer(I4P), intent(in)            :: ror_schemes_gpu(1:)
   real(R8P),    intent(in)            :: q_aux_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer(I4P), intent(inout)         :: ror_stats_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:)
   real(R8P),    intent(inout)         :: flx_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer                             :: b, i, j, k, l, ll, m, mm, v
   real(R8P)                           :: er(5,5), el(5,5), ev(5), evmax(5), ghat(5), gl(5), gr(5), fi(5), vi(5), gp(5,8), gm(5,8)
      ...
      do j=1,nj
         do i=0,ni ! loop on faces
            do b=1,blocks_number
               ...
               call weno_reconstruction_kernel(nvar=nv, vp=gp, vm=gm, vminus=gl, vplus=gr, iweno=iweno, wenorec_ord=wenorec_scheme)

               ror_x: do m = 2, size(ror_schemes_gpu)
                  ror_to_recompute = .false.
                  do mm = 1,size(ror_ivar_gpu)
                      index_var = ror_ivar_gpu(mm)
                      if((abs(gl(index_var)-gp(index_var,iweno))   > ror_threshold*abs(gp(index_var,iweno))   ) .or. &
                         (abs(gr(index_var)-gm(index_var,iweno+1)) > ror_threshold*abs(gm(index_var,iweno+1))) ) then
   ...
   subroutine compute_flux_conv_y_kernel(blocks_number, ni, nj, nk, ngc, nv, iweno, dha, g, R, cv,       &
                                         ror_threshold, enable_ror_stats, cell_scheme_gpu, ror_ivar_gpu, &
                                         ror_schemes_gpu, q_aux_gpu, ror_stats_gpu, fly_gpu)
   ...
   real(R8P),    intent(in)            :: q_aux_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer(I4P), intent(inout)         :: ror_stats_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:)
   real(R8P),    intent(inout)         :: fly_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer                             :: b, i, j, k, l, ll, m, mm, v
   real(R8P)                           :: er(5,5), el(5,5), ev(5), evmax(5), ghat(5), gl(5), gr(5), fi(5), vi(5), gp(5,8), gm(5,8)
   real(R8P)                           :: uu, vv, ww, h, ya, qq, c, ci, b1, b2
   real(R8P)                           :: gc, wc
   ...
   subroutine compute_flux_conv_z_kernel(blocks_number, ni, nj, nk, ngc, nv, iweno, dha, g, R, cv,       &
                                         ror_threshold, enable_ror_stats, cell_scheme_gpu, ror_ivar_gpu, &
                                         ror_schemes_gpu, q_aux_gpu, ror_stats_gpu, flz_gpu)
   ...
   real(R8P),    intent(in)            :: q_aux_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer(I4P), intent(inout)         :: ror_stats_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:)
   real(R8P),    intent(inout)         :: flz_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   integer                             :: b, i, j, k, l, ll, m, mm, v
   real(R8P)                           :: er(5,5), el(5,5), ev(5), evmax(5), ghat(5), gl(5), gr(5), fi(5), vi(5), gp(5,8), gm(5,8)
   real(R8P)                           :: uu, vv, ww, h, ya, qq, c, ci, b1, b2
   real(R8P)                           :: gc, wc
   ...
   real(R8P),    intent(in)            :: fc_coeff_gpu(1:, 1:)
   real(R8P),    intent(in)            :: q_aux_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   real(R8P),    intent(in)            :: dx_gpu(1:)
   real(R8P),    intent(inout)         :: flx_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   ...
   !$omp target teams distribute parallel do collapse(4) has_device_addr(fc_coeff_gpu,q_aux_gpu,dx_gpu,flx_gpu)
   do k=1,nk
      do j=1,nj
         do i=0,ni ! loop on faces
            do b=1,blocks_number
   ...
   !$omp target teams distribute parallel do collapse(4) has_device_addr(fc_coeff_gpu,q_aux_gpu,dy_gpu,fly_gpu)
   do k=1,nk
      do j=0,nj ! loop on faces
         do i=1,ni
            do b=1,blocks_number
   ...
   !$omp target teams distribute parallel do collapse(4) has_device_addr(fc_coeff_gpu,q_aux_gpu,dz_gpu,flz_gpu)
   do k=0,nk ! loop on faces
      do j=1,nj
         do i=1,ni
            do b=1,blocks_number
   ...
   real(R8P),    intent(in)    :: phi_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< IB distance function.
   real(R8P),    intent(inout) ::  fl_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Fluxes differences.
   ...
   !$omp&                                                                flx_gpu,fly_gpu,flz_gpu,phi_gpu,fl_gpu)
   do k=1,nk
   do j=1,nj
   do i=1,ni
   do b=1,blocks_number
   ...
   real(R8P),    intent(in)    :: dx_gpu(1:), dy_gpu(1:), dz_gpu(1:)
   real(R8P),    intent(in)    :: q_aux_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   real(R8P),    intent(inout) ::   flx_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   real(R8P),    intent(inout) ::   fly_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   real(R8P),    intent(inout) ::   flz_gpu(1:, 1-ngc:, 1-ngc:, 1-ngc:, 1:)
   ...
   !$omp target teams distribute parallel do collapse(3) has_device_addr(dx_gpu,dy_gpu,dz_gpu,&
   ...
   gradient = 0._R8P
   !$omp target teams distribute parallel do collapse(3) has_device_addr(q_gpu) reduction(max:gradient)
   do k=1, nk
      do j=1, nj
         do i=1, ni
      ...
      enddo
   enddo
   endsubroutine compute_q_gradient_gmp
   ...
   subroutine compute_roe_average(q_aux_gpu, dha, g, &
   ...
   !$omp declare target(compute_roe_average)
   ! Left state (node i)
   ...
   subroutine compute_rk_q_gpu_gmp(ni,nj,nk,ngc,nv,blocks_number,dt,q_gpu,q_old_gpu,fl_gpu,phi_gpu,ark,brk,crk)
   ...
   real(R8P),    intent(in)             ::    fl_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Conservative field.
   real(R8P),    intent(in),   optional ::   phi_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< Conservative field.
   real(R8P),    intent(in)             :: q_old_gpu(1:,1-ngc:,1-ngc:,1-ngc:,1:) !< RK stage.
   integer(I4P)                         :: all_solids                            !< Last phi index, all solids summary.
   integer(I4P)                         :: i, j, k, b, v                         !< Counter.

   if (present(phi_gpu)) then
      all_solids = ubound(phi_gpu, dim=5)
      !$omp target teams distribute parallel do collapse(5) has_device_addr(q_gpu,fl_gpu,phi_gpu,q_old_gpu)
      do v=1, nv
         do k=1, nk
            do j=1, nj
               do i=1, ni
   ...
   ! WENO procedures
   subroutine weno_reconstruction_kernel(nvar, vp, vm, vminus, vplus, iweno, wenorec_ord)
   ...
   real(R8P)                               :: x,y,y2
   !$omp declare target(weno_reconstruction_kernel)
