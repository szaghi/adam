!< ADAM, fluid physics  class definition, CPU backend.
module adam_nasto_physics_object
!< ADAM, fluid physics  class definition, CPU backend.
...
character(len=15), parameter :: INI_SECTION_NAME='physics_specie_' !< INI file section name containing fluid physics.

type :: nasto_physics_object
   !< Physics of ideal compressible fluid object class.
   ...
   contains
      ...
      ! operators
      generic :: assignment(=) => nasto_physics_assign_nasto_physics !< Overload `=`.
      ! private methods
      procedure, pass(lhs), private :: nasto_physics_assign_nasto_physics !< Operator `=`.
endtype nasto_physics_object

interface nasto_physics_object
   !< Overload [[nasto_physics_object]] name with its constructor.
   module procedure nasto_physics_instance
endinterface
contains
   ! public methods
   elemental subroutine compute_derivate(self)
   !< Compute derivate quantities (from `cp` and `cv`).
   class(nasto_physics_object), intent(inout) :: self !< Fluid physics.
   ...
   class(nasto_physics_object), intent(in) :: self            !< Fluid physics.
   ...
   !< Destroy physics.
   class(nasto_physics_object), intent(inout) :: self  !< Fluid physics.
   type(nasto_physics_object)                 :: fresh !< Fresh fluid physics.
   ...
   class(nasto_physics_object), intent(inout)        :: self    !< Fluid physics.
   type(nasto_physics_object),  intent(in), optional :: physics !< Fluid physics.
   ...
   if (present(physics)) then
      self = physics
   else
      self = nasto_physics_object(mu=mu, kd=kd, dha=dha, cp=cp, cv=cv, gam=gam, R=R)
   endif
   endsubroutine initialize

   subroutine load_from_file(self, fini, s, go_on_fail)
   !< Load from file.
   class(nasto_physics_object), intent(inout)        :: self        !< Fluid physics.
   ...
   subroutine save_into_file(self, fini, s)
   !< Save into file.
   class(nasto_physics_object), intent(inout) :: self  !< Free conditions.
   ...
   call fini%add(section_name=sname, option_name='cp',  val=self%cp )
   call fini%add(section_name=sname, option_name='cv',  val=self%cv )
   call fini%add(section_name=sname, option_name='mu',  val=self%mu )
   call fini%add(section_name=sname, option_name='kd',  val=self%kd )
   call fini%add(section_name=sname, option_name='dha', val=self%dha)
   ...
   ! operators
   pure subroutine nasto_physics_assign_nasto_physics(lhs, rhs)
   !< Operator `=`.
   class(nasto_physics_object), intent(inout) :: lhs !< Left hand side.
   type(nasto_physics_object),  intent(in)    :: rhs !< Right hand side.
   ...
   lhs%kd    = rhs%kd
   lhs%dha   = rhs%dha
   endsubroutine nasto_physics_assign_nasto_physics
   ...
   ! private non TBP
   elemental function nasto_physics_instance(mu, kd, dha, cp, cv, gam, R) result(instance)
   !< Return and instance of [[nasto_physics_object]].
   !<
   !< @note This procedure is used for overloading [[nasto_physics_object]] name.
   ...
   real(R8P), intent(in), optional :: gam      !< Specific heats ratio `gamma=cp/cv` value.
   real(R8P), intent(in), optional :: R        !< Fluid constant `R=cp-cv` value.
   type(nasto_physics_object)      :: instance !< Instance of [[nasto_physics_object]].
   ...
   endfunction nasto_physics_instance
endmodule adam_nasto_physics_object
