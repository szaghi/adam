!< ADAM, PATCH IO handler class definition, common CPU backend.
module adam_patch_io_object
...
endtype patch_io_object

contains
   ! public methods
   pure function description(self) result(desc)
   !< Return a pretty-formatted object description.
   class(patch_io_object), intent(in) :: self             !< IO handler.
   ...
   class(patch_io_object), intent(inout) :: self     !< IO handler.
   character(*),           intent(in)    :: filename !< File name of parameters file.

   call self%mpih%initialize(do_mpi_init=.false.)
   print '(A)', self%mpih%myrankstr//'patch_io_object%initialize start'
   ...
   print '(A)', self%mpih%myrankstr//'patch_io_object%initialize finish'
...
endmodule adam_patch_io_object
