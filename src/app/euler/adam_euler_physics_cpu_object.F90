...
module adam_euler_physics_cpu_object
!< ADAM, Physics class definition, CPU backend.
...
implicit none
private
public :: euler_physics_cpu_object
public :: IR
public :: IU
public :: IV
public :: IW
...
endtype euler_physics_cpu_object
...
contains
   ...
   class(euler_physics_cpu_object), intent(in) :: self                   !< Equation of state.
   ...
   class(euler_physics_cpu_object), intent(in) :: self             !< Physics.
   character(len=:), allocatable               :: desc             !< Description.
   character(len=1), parameter                 :: NL=new_line('a') !< New line character.
   integer(I4P)                                :: s                !< Counter.

   desc =       self%mpih%myrankstr//'euler_physics_cpu_object ns:     '//trim(str(self%ns    ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object nv:     '//trim(str(self%nv    ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object nv_aux: '//trim(str(self%nv_aux))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object np:     '//trim(str(self%np    ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IR:     '//trim(str(IR         ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IU:     '//trim(str(IU         ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IV:     '//trim(str(IV         ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IW:     '//trim(str(IW         ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IG:     '//trim(str(IG         ))//NL
   desc = desc//self%mpih%myrankstr//'euler_physics_cpu_object IP:     '//trim(str(IP         ))
   do s=1, self%ns
      desc = desc//NL//self%mpih%myrankstr//'euler_physics_cpu_object cp('//trim(str(s,.true.))//'):  '//trim(str(self%eos(s)%cp))
      desc = desc//NL//self%mpih%myrankstr//'euler_physics_cpu_object cv('//trim(str(s,.true.))//'):  '//trim(str(self%eos(s)%cv))
      desc = desc//NL//self%mpih%myrankstr//'euler_physics_cpu_object  g('//trim(str(s,.true.))//'):  '//trim(str(self%eos(s)%g ))
      desc = desc//NL//self%mpih%myrankstr//'euler_physics_cpu_object  R('//trim(str(s,.true.))//'):  '//trim(str(self%eos(s)%R ))
   enddo
   endfunction description

   subroutine initialize(self, file_parameters)
   !< Initialize the equation.
   class(euler_physics_cpu_object), intent(inout) :: self            !< Physics.
   type(file_ini),                  intent(in)    :: file_parameters !< Simulation parameters ini file handler.

   call self%mpih%initialize(do_mpi_init=.false.)
   print '(A)', self%mpih%myrankstr//'euler_physics_cpu_object%initialize start'
   ...
   print '(A)', self%mpih%myrankstr//'euler_physics_cpu_object%initialize finish'
...
endmodule adam_euler_physics_cpu_object
