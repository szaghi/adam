...
endtype slices_cpu_object

contains
   ! public methods
   subroutine initialize(self, file_parameters)
   !< Initialize the equation.
   class(slices_cpu_object), intent(inout) :: self            !< Slices.
   type(file_ini),           intent(in)    :: file_parameters !< Simulation parameters ini file handler.

   call self%mpih%initialize(do_mpi_init=.false.)
   print '(A)', self%mpih%myrankstr//'slice_cpu_object%initialize start'
   call self%load_from_file(file_parameters=file_parameters)
   print '(A)', self%mpih%myrankstr//'slice_cpu_object%initialize finish'
   endsubroutine initialize

   pure function is_to_save(self, it, it_max, time, time_max)
   !< Return true if slices are to save.
   class(slices_cpu_object), intent(in) :: self       !< Slices.
...
endmodule adam_slices_cpu_object
