...
module adam_chase_common_object
!< ADAM, CHASE (CFD-HPC enabled-Adaptive mesh-Simulation code for-Euler equations) class definition, common CPU backend.
...
implicit none
private
public :: chase_common_object

type :: chase_common_object
   ...
   ! CHASE library objects
   type(chase_io_object)      :: io      !< IO handler.
   type(chase_physics_object) :: physics !< Fluids physiscs handler.
   type(chase_ic_object)      :: ic      !< Initial Conditions (IC) handler.
   type(chase_bc_object)      :: bc      !< Boundary Conditions (BC) handler.
   type(chase_time_object)    :: time    !< Time handler.
   ...
   character(2), allocatable :: q_name(:)        !< Conservative fields names (r,ru,rv,rw,rE).
   contains
...
endtype chase_common_object
contains
   subroutine allocate_common(self)
   !< Allocate common data.
   class(chase_common_object), intent(inout) :: self !< The equation.
   ...
   !                                     1-ngc,nk+ngc, &
   !                                     1,nb],[2,5]), &
   !                        msg=self%mpih%myrankstr//'prsim_common_object%allocate_common(q) ', verbose=.true.)
   ! self%q = 0._R8P
   call allocate_variable(var=self%q_aux,            &
                          ulb=reshape([1,self%nv_aux,&
                                       1-ngc,ni+ngc, &
                                       1-ngc,nj+ngc, &
                                       1-ngc,nk+ngc, &
                                       1,nb],[2,5]), &
                          msg=self%mpih%myrankstr//'prsim_common_object%allocate_common(q_aux) ', verbose=.true.)
   self%q_aux = 0._R8P
   endassociate
   ...
   class(chase_common_object), intent(inout), target :: self         !< The equation.
   ...
   if (verbose_) call self%mpih%print_message('chase_common_object%initialize start')
   ...
   call self%adam%io%initialize(grid=self%adam%grid, field=self%adam%field, &
                                q1_R8P=self%q_aux, q1_R8P_name=['r_a','u_a','v_a','w_a','p_a','a_a','T_a','E_a','H_a','Rfa','g_a'])
   self%q_name = ['r ','ru','rv','rw','rE']
   endassociate
   if (verbose_) call self%mpih%print_message('chase_common_object%initialize finish')
   contains
      ...
      type(chase_physics_object), intent(in), target :: physics !< The physics.
...
endmodule adam_chase_common_object
