...
do o=2, 10, 2
   ...
   call save_file(basename='fd_laplacian_sin3-ord', order=o)
enddo
...
contains
   ...
   do j=0, n
   do i=0, n
      write(fu,'(A)') trim(str(nodes(1,i,j,k)))
   enddo
   enddo
   enddo
   do k=0, n
   do j=0, n
   do i=0, n
      write(fu,'(A)') trim(str(nodes(2,i,j,k)))
   enddo
   enddo
   enddo
   do k=0, n
   do j=0, n
   do i=0, n
      write(fu,'(A)') trim(str(nodes(3,i,j,k)))
   enddo
   enddo
   enddo
   do k=1, n
