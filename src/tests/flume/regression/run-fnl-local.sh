#!/usr/bin/env bash
# Local FLUME FNL regression trigger (issue #35, P8) — workstation-only convenience wrapper, a copy of the PRISM one.
#
...
#
# Usage:
#   ./run-fnl-local.sh                # build flume-fnl-nvf, run, diff FNL golden
...
#   ./run-fnl-local.sh                # produces sod-x/work-fnl/digest.txt
#   mkdir -p sod-x/golden/fnl
#   cp sod-x/work-fnl/digest.txt sod-x/work-fnl/*-residuals.dat sod-x/work-fnl/*-conservation_history.dat sod-x/golden/fnl/
#
...
#   UCX_MEMTYPE_CACHE=n            WSL memory-type detection is unreliable.
