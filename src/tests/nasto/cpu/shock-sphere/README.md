...
# Shock-sphere test for NASTO CPU

> Shock-sphere interaction test for ADAM NASTO app, CPU backend.

This test perform a shock-sphere interaction simulation of ADAM NASTO app compiled with CPU backend.
...
To install and compile NASTO CPU app see [its own documentation](../../../../app/nasto/cpu/README.md).

Once NASTO CPU app has been compiled, the test is ready to be exectuted.
...
```bash
┌╼ stefano@enlil
├───╼ ~/fortran/adam/src/tests/nasto/cpu/shock-sphere
└──────╼ tree
.
├── adam_nasto_cpu -> ../../../../../exe/adam_nasto_cpu
...
+ `adam_nasto_cpu` is a symbolic link to the NASTO CPU app that should be correctly placed if the [install](#install) step has been correctly completed;
