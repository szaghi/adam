...
# Shock-sphere test for NASTO GMP

> Shock-sphere interaction test for ADAM NASTO app, GMP backend.

This test perform a shock-sphere interaction simulation of ADAM NASTO app compiled with GMP backend. The test is configured to run almost quickly
...
To install and compile NASTO GMP app see [its own documentation](../../../../app/nasto/gmp/README.md).

Once NASTO GMP app has been compiled, the test is ready to be exectuted.
...
```bash
┌╼ stefano@enlil
├───╼ ~/fortran/adam/src/tests/nasto/gmp/shock-sphere
└──────╼ tree
.
├── adam_nasto_gmp -> ../../../../../exe/adam_nasto_gmp
...
+ `adam_nasto_gmp` is a symbolic link to the NASTO GMP app that should be correctly placed if the [install](#install) step has been correctly completed;
