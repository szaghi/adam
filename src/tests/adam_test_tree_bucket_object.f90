...
call tree_bucket%add_node(code=1_I8P, content=1_I8P)
call tree_bucket%add_node(code=2_I8P, content=2_I8P)
call tree_bucket%add_node(code=3_I8P, content=3_I8P)
call tree_bucket%add_node(code=4_I8P, content=4_I8P)
call tree_bucket%add_node(code=5_I8P, content=5_I8P)
call tree_bucket%add_node(code=6_I8P, content=6_I8P)

print '(A)', 'loop into tree bucket'
do while(tree_bucket%loop(code=code, content=content))
   print '(A)', ' node: "'//trim(str(code))//'" ='//trim(str(content))
enddo
...
print '(A)', ' node: "3" ='//trim(str(tree_node%content))
