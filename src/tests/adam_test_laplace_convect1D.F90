!< ADAM, test ADAM for Laplace/convective 1D equations.
program adam_test_laplace_convect1D
!< ADAM, test ADAM for Laplace/convective 1D equations.

use adam_adam_object
use adam_equation_convect1D_cpu_object, only : equation_convect1D_cpu_object
use adam_equation_laplace_cpu_object
use adam_parameters
use PENF

implicit none

type(adam_object)                   :: adam            !< ADAM.
type(equation_convect1D_cpu_object) :: convect         !< 1D convection equation.
type(equation_laplace_cpu_object)   :: laplace         !< Laplace equation.
integer(I4P)                        :: l, t            !< Counter.
logical                             :: is_grid_changed !< Flag to check grid changes.
real(R8P)                           :: time            !< Time.
integer(I4P)                        :: n_iter=100      !< Number of iterations.
integer(I4P)                        :: n_save=1        !< Frequency of saving output.

!TEST1 print '(A)', 'Laplace equation integration'
!TEST1 call adam%initialize(max_level=7,                                              &
!TEST1                      emin=[0._R8P,0._R8P,0._R8P], emax=[1._R8P,1._R8P,1._R8P], &
!TEST1                      ni=8_I4P, nj=8_I4P, nk=8_I4P, gc=[2_I4P,2_I4P,2_I4P],     &
!TEST1                      bc_type=[BC_INFLOW,BC_EXTRAPOLATION,                      &
!TEST1                               BC_EXTRAPOLATION,BC_EXTRAPOLATION,               &
!TEST1                               BC_EXTRAPOLATION,BC_EXTRAPOLATION],              &
!TEST1                      nb=5000, nodes_number=16*5000_I8P)
!TEST1 call laplace%initialize(field=adam%field)
!TEST1 print '(A)', 'create 2 levels of refinement'
!TEST1 do l=1, 2
!TEST1    print '(A)', 'refine ADAM at level '//trim(str(l))
!TEST1    print *, 'blocks_number: ',adam%field%blocks_number
!TEST1    call adam%tree%mark_all_nodes(mark=TO_BE_REFINED)
!TEST1    call laplace%update_ghost(q=adam%field%q)
!TEST1    call adam%amr_update(do_blocks_reorder=.false., do_mpi_redistribute=.true.)
!TEST1 enddo
!TEST1 print '(A)', 'set initial conditions'
!TEST1 call laplace%set_initial_conditions
!TEST1 call adam%save_hdf5(basename='laplace-'//trim(strz(0,9)), with_ghost=.false., with_cell_morton=.true.)
!TEST1 time = 0._R8P
!TEST1 do t=1, n_iter
!TEST1    if (mod(t,1)==0.and.adam%myrank==0) print '(A)', 'track iteration '//trim(str(t, .true.))
!TEST1    call laplace%mark_by_grad_q
!TEST1    call laplace%update_ghost(q=adam%field%q)
!TEST1    call adam%amr_update(is_marked_by_field=.true., do_blocks_reorder=.false.)
!TEST1    if (mod(t,1)==0.and.adam%myrank==0) print '(A)', 'blocks number: '//trim(str(adam%tree%nodes_number, .true.))
!TEST1    call laplace%integrate(t=time, Dt=0.000001_R8P)
!TEST1    if (mod(t,n_save)==0) call adam%save_hdf5(basename='laplace-'//trim(strz(t,9)), with_ghost=.false., with_cell_morton=.true.)
!TEST1    time = time + 0.2_R8P
!TEST1 enddo
!TEST1 
!TEST1 print '(A)', ''

print '(A)', '1D convection equation integration'
call adam%initialize(max_level=7,                                              &
                     emin=[0._R8P,0._R8P,0._R8P], emax=[1._R8P,1._R8P,1._R8P], &
                     ni=8_I4P, nj=8_I4P, nk=8_I4P, gc=[2_I4P,2_I4P,2_I4P],     &
                     bc_type=[BC_PERIODIC,BC_PERIODIC,                         &
                              BC_EXTRAPOLATION,BC_EXTRAPOLATION,               &
                              BC_EXTRAPOLATION,BC_EXTRAPOLATION],              &
                     nb=5000, nodes_number=16*5000_I8P)
call convect%initialize(field=adam%field)
print '(A)', 'create 2 levels of refinement'
do l=1, 2
   print '(A)', 'refine ADAM at level '//trim(str(l))
   print *, 'blocks_number: ',adam%field%blocks_number
   call adam%tree%mark_all_nodes(mark=TO_BE_REFINED)
   call convect%update_ghost(q=adam%field%q)
   call adam%amr_update(do_blocks_reorder=.false., do_mpi_redistribute=.true.)
enddo
print '(A)', 'set initial conditions'
call convect%set_initial_conditions
call adam%save_hdf5(basename='convect-'//trim(strz(0,9)), with_ghost=.false., with_cell_morton=.true.)
time = 0._R8P
do t=1, n_iter
   if (mod(t,1)==0.and.adam%myrank==0) print '(A)', 'track iteration '//trim(str(t, .true.))
   call convect%mark_by_grad_q
   call convect%update_ghost(q=adam%field%q)
   call adam%amr_update(is_marked_by_field=.true., do_blocks_reorder=.false.)
   if (mod(t,1)==0.and.adam%myrank==0) print '(A)', 'blocks number: '//trim(str(adam%tree%nodes_number, .true.))
   call convect%integrate(t=time, Dt=0.006_R8P)
   if (mod(t,n_save)==0) call adam%save_hdf5(basename='convect-'//trim(strz(t,9)), with_ghost=.false., with_cell_morton=.true.)
   time = time + 0.2_R8P
enddo

call adam%finalize
endprogram adam_test_laplace_convect1D
