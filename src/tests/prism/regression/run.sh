...
# directory named by the executable (adam_prism_cpu -> cpu/, adam_prism_fnl -> fnl/; other applications' and other
# backends' sources do not enter it). Also guards --no-build runs.
backend_dir="${EXE##*_}"
source_dirs=("$REPO_ROOT/src/lib/common" "$REPO_ROOT/src/third_party" "$REPO_ROOT/src/app/prism/common" \
             "$REPO_ROOT/src/app/prism/$backend_dir")
